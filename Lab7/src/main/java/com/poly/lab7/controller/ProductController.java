...
public class ProductController {
    ...
    @RequestMapping("/product/searchByDSL")
    public String findByPriceBetween(Model model,
                         @RequestParam("min") Optional<Double> min,
                         @RequestParam("max") Optional<Double> max) {
        double minPrice = min.orElse(Double.MIN_VALUE);
        double maxPrice = max.orElse(Double.MAX_VALUE);
        ...
        model.addAttribute("items", items);
        return "product/searchByDSL";
    }

    @RequestMapping("/product/search-and-pageByDSL")
    public String searchAndPageByDSL(Model model,
                                @RequestParam("keywords") Optional<String> kw,
                                @RequestParam("p") Optional<Integer> p) {
        String kwords = kw.orElse(session.get("keywords", ""));
        session.set("keywords", kwords);
        ...
        model.addAttribute("page", page);
        return "product/search-and-pageByDSL";
    }
}
